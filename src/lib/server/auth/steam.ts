...
export async function authenticateSteamCallback(url: string) {
  return new Promise<PlayerSummary>((resolve, reject) => {
    openID.verifyAssertion(url, async (err, result) => {
      ...
      if (result === null || result?.claimedIdentifier === undefined)
        return reject(new Error('Authentication failed - No result returned from Steam'));
      if (result?.authenticated !== true)
        return reject(new Error('Authentication failed - Steam returned an unauthenticated result'));
